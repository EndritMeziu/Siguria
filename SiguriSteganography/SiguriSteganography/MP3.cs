...
namespace steganography
{
	...
	public sealed class MP3
	{
		...
		public MP3(string fileName)
		{
            ...
            if (File.Exists(fileName))
            {
                ...
                if (contentBytes[0] == 'I' && contentBytes[1] == 'D' && contentBytes[2] == '3')
                {
                    ...
                    {
                        for (int l = 0; l < 7; l++)
                        {   //msb vendoset ne zero dhe nuk merret parasysh 8,16,24
                    ...
                    int theSize_int = 0;
                    //llogaritja e madhesise se tagut pa header 
                    for (int k = 0; k < 32; k++)
                    {
                        int pow = 1;
                        for (int l = 1; l <= k; l++)
            ...
            {
                Console.WriteLine(" Error File Input.\n");
		...
		{
			int messageSize = arrMsgByte.Length * 8; //madhesia e mesazhit ne bita
			string strMsgLength = Convert.ToString(messageSize);

			if (arrMsgByte.Length * 8 * lsb > ContentSize_bits)// > bitat e fajllit 
			{
                
				throw new Exception("Error : " + arrMsgByte.Length * 8 * lsb + " bitat " + ContentSize_bits);
			}
		...
		public BitSet parser()
		{
			...
			{                                                         //dyjat vlera pozitive vlen per qdo  
				if ((contentBytes[contentBytes.Length - i / 8 - 1] & (1 << (i % 8))) > 0) //numer qe ka 1 ne poziten e njejte me i-ne 
				{
                    theMP3Bits.Set(i);
		...
		public byte[] toByteArray(BitSet bits)
		{
			...
			for (int i = 0; i < bits.Size; i++)
			{
				if (bits.Get(i))
				{
					bytes[bytes.Length - i / 8 - 1] |= (byte)(1 << (i % 8));//bitwise or
				}
			}
			return bytes;
		}
		...
		public string decoder(int lsb)
		{
			...
			BitSet theChar_bits = new BitSet(8);//leximi i nga 8 bitave ne unaze
			BitSet theMessage;
			string taille = "";
			byte[] theChar;
			...
			for (int j = 0; j < int.Parse(taille); j++) 
			{
				if (theContent_bits.Get(currentPosisition))
				{
